...
const {
  ...
  checkOrigin,
  ab2str
} = require("./utils"); // NOTE: throws if origin is https and has port 443
...
function parseClientResponse(msg) {
  ...
  var clientDataJson = ab2str(clientDataJSON); // parse JSON string
  ...
  var ret = new Map([["challenge", parsed.challenge], ["origin", parsed.origin], ["type", parsed.type], ["tokenBinding", parsed.tokenBinding], ["rawClientDataJson", clientDataJSON], ["rawId", rawId]]);
  return ret;
}
/**
...
function parseAttestationObject(attestationObject) {
  ...
  attestationObject = coerceToArrayBuffer(attestationObject, "attestationObject"); // parse attestation
  ...
  var ret = new Map([...Fido2Lib.parseAttestation(parsed.fmt, parsed.attStmt), // return raw buffer for future signature verification
  ["rawAuthnrData", coerceToArrayBuffer(parsed.authData, "authData")], // parse authData
...
function parseAuthenticatorData(authnrDataArrayBuffer) {
  ...
  var ret = new Map(); // console.log("authnrDataArrayBuffer", authnrDataArrayBuffer);
  ...
  offset += 4; // see if there's more data to process
...
function parseAuthnrAssertionResponse(msg) {
  ...
  let ret = new Map([["sig", sigAb], ["userHandle", userHandle], ["rawAuthnrData", msg.response.authenticatorData], ...parseAuthenticatorData(msg.response.authenticatorData)]);
  return ret;
}
