...
function fidoU2fValidateFn() {
  return _fidoU2fValidateFn.apply(this, arguments);
}

function _fidoU2fValidateFn() {
  _fidoU2fValidateFn = _asyncToGenerator(function* () {
    var x5c = this.authnrData.get("x5c");
    var parsedAttCert = this.authnrData.get("attCert"); // validate cert chain

    if (x5c.length > 0) {
      throw new Error("cert chain not validated");
    }

    this.audit.journal.add("x5c"); // make sure our root certs are loaded
    ...
    this.audit.journal.add("attCert"); // https://w3c.github.io/webauthn/#fido-u2f-attestation
    ...
    var credId = this.authnrData.get("credId"); // create clientDataHash
    ...
    var clientDataHash = new Uint8Array(clientDataHashBuf).buffer; // Convert the COSE_KEY formatted credentialPublicKey (see Section 7 of [RFC8152]) to CTAP1/U2F public Key format [FIDO-CTAP]
    ...
    var verificationData = new Uint8Array([0x00, ...new Uint8Array(rpIdHash), ...new Uint8Array(clientDataHash), ...new Uint8Array(credId), 0x04, ...new Uint8Array(x), ...new Uint8Array(y)]); // Verify the sig using verificationData and certificate public key per [SEC1].
    ...
    this.audit.journal.add("sig"); // If successful, return attestation type Basic with the attestation trust path set to x5c.
