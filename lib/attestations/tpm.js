...
function tpmParseFn(attStmt) {
  ...
  ret.set("attCert", newX5c.shift()); // the rest of the certificates (if any) are the certificate chain

  ret.set("x5c", newX5c); // ecdaa

  if (attStmt.ecdaaKeyId) ret.set("ecdaaKeyId", attStmt.ecdaaKeyId); // sig

  ret.set("sig", coerceToArrayBuffer(attStmt.sig, "tpm signature")); // sig

  ret.set("ver", attStmt.ver); // alg

  var alg = {
    algName: coseToJwk.algToStr(attStmt.alg),
    hashAlg: coseToJwk.algToHashStr(attStmt.alg)
  };
  ret.set("alg", alg); // certInfo

  var certInfo = parseCertInfo(coerceToArrayBuffer(attStmt.certInfo, "certInfo"));
  ret.set("certInfo", certInfo); // pubArea
...
function parseCertInfo(certInfo) {
  ...
  var ret;
  var ci = new Map();
  ci.set("rawCertInfo", certInfo); // TPM_GENERATED_VALUE magic number

  var magic = dv.getUint32(offset); // if this isn't the magic number, the rest of the parsing is going to fail
  ...
  ci.set("magic", magic);
  offset += 4; // TPMI_ST_ATTEST type

  var type = decodeStructureTag(dv.getUint16(offset)); // if this isn't the right type, the rest of the parsing is going to fail
...
function parsePubArea(pubArea) {
  ...
  var ret;
  var pa = new Map();
  pa.set("rawPubArea", pubArea); // TPMI_ALG_PUBLIC type

  var type = algIdToStr(dv.getUint16(offset));
  pa.set("type", type);
  offset += 2; // TPMI_ALG_HASH nameAlg

  pa.set("nameAlg", algIdToStr(dv.getUint16(offset)));
  offset += 2; // TPMA_OBJECT objectAttributes

  pa.set("objectAttributes", decodeObjectAttributes(dv.getUint32(offset)));
  offset += 4; // TPM2B_DIGEST authPolicy

  ret = getSizedElement(dv, offset);
  pa.set("authPolicy", ret.buf);
  offset = ret.offset; // TPMU_PUBLIC_PARMS parameters
  ...
  offset += 4; // TPMU_PUBLIC_ID unique
...
function algIdToStr(hashType) {
  var hashList = ["TPM_ALG_ERROR", // 0
  "TPM_ALG_RSA", // 1
  null, null, "TPM_ALG_SHA1", // 4
  "TPM_ALG_HMAC", // 5
  "TPM_ALG_AES", // 6
  "TPM_ALG_MGF1", // 7
  null, "TPM_ALG_KEYEDHASH", // 8
  "TPM_ALG_XOR", // A
  "TPM_ALG_SHA256", // B
  "TPM_ALG_SHA384", // C
  "TPM_ALG_SHA512", // D
  null, null, "TPM_ALG_NULL", // 10
  null, "TPM_ALG_SM3_256", // 12
  "TPM_ALG_SM4", // 13
  "TPM_ALG_RSASSA", // 14
  "TPM_ALG_RSAES", // 15
  "TPM_ALG_RSAPSS", // 16
...
function tpmValidateFn() {
  return _tpmValidateFn.apply(this, arguments);
}

function _tpmValidateFn() {
  _tpmValidateFn = _asyncToGenerator(function* () {
    ...
    this.audit.journal.add("ver"); // https://www.w3.org/TR/webauthn/#tpm-attestation
    ...
    this.audit.journal.add("pubArea"); //     Note that the remaining fields in the "Standard Attestation Structure" [TPMv2-Part1] section 31.2, i.e., qualifiedSigner, clockInfo and firmwareVersion are ignored.
    ...
    this.audit.journal.add("certInfo"); //     Verify that x5c meets the requirements in §8.3.1 TPM attestation statement certificate requirements.
    ...
    this.audit.journal.add("attCert"); //     If successful, return attestation type AttCA and attestation trust path x5c.

    this.audit.info.set("attestation-type", "AttCA");
    this.audit.journal.add("fmt");
    return true; // If ecdaaKeyId is present, then the attestation type is ECDAA.
