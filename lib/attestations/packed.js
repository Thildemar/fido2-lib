...
function packedValidateFn() {
  return _packedValidateFn.apply(this, arguments);
}

function _packedValidateFn() {
  _packedValidateFn = _asyncToGenerator(function* () {
...
function packedValidateBasic() {
  return _packedValidateBasic.apply(this, arguments);
}

function _packedValidateBasic() {
  _packedValidateBasic = _asyncToGenerator(function* () {
    ...
    var res = validateSignature(this.clientData.get("rawClientDataJson"), this.authnrData.get("rawAuthnrData"), this.authnrData.get("sig"), hashAlg, this.authnrData.get("attCert"));
    ...
    this.audit.journal.add("alg"); // Verify that x5c meets the requirements in §8.2.1 Packed attestation statement certificate requirements.

    yield validateCerts(this.authnrData.get("attCert"), this.authnrData.get("aaguid"), this.authnrData.get("x5c"), this.audit); // If successful, return attestation type Basic and attestation trust path x5c.
...
function validateSignature(rawClientData, authenticatorData, sig, hashAlg, parsedAttCert) {
  ...
  var clientDataHash = new Uint8Array(clientDataHashBuf).buffer; // convert cert to PEM

  var attCertPem = abToPem("CERTIFICATE", parsedAttCert); // verify signature
...
function validateCerts(_x, _x2, _x3, _x4) {
  return _validateCerts.apply(this, arguments);
}

function _validateCerts() {
  _validateCerts = _asyncToGenerator(function* (parsedAttCert, aaguid, x5c, audit) {
    ...
    audit.journal.add("x5c"); // cert MUST be x.509v3
    ...
    audit.journal.add("attCert"); // console.log("_cert", attCert._cert);
