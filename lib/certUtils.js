...
const {
  ...
  coerceToArrayBuffer,
  ab2str
} = require("./utils"); // install crypto engine in pkijs
...
class Certificate {
  constructor(cert) {
    ...
    this._cert = new PkijsCertificate({
      schema: asn1.result
    });
...
function resolveOid(id, value) {
  ...
  switch (id) {
    ...
    case "2.5.29.35":
      ...
      retMap.set("key-identifier", decodeValue(value.keyIdentifier)); // TODO: other values
...
function decodeValue(valueBlock) {
  var blockType = Object.getPrototypeOf(valueBlock).constructor.name; // console.log("blockType", blockType);
...
function decodeAuthorityInfoAccess(v) {
  ...
  v.accessDescriptions.forEach(desc => {
    var {
      id,
      value
    } = resolveOid(desc.accessMethod, desc.accessLocation);
    retMap.set(id, value);
  });
  return retMap;
}
    ...
    case 4:
      ...
      v.forEach(val => {
        var {
          id,
          value
        } = resolveOid(val.type, decodeValue(val.value));
...
class CRL {
  constructor(crl) {
    ...
    this._crl = new PkijsCertificateRevocationList({
      schema: asn1.result
    });
  }
...
class CertManager {
  ...
  static verifyCertChain(certs, roots, crls) {
