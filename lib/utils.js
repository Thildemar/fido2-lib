...
function coerceToBase64(thing, name) {
  ...
  if (typeof thing === "object" && thing.buffer instanceof ArrayBuffer && !(thing instanceof Buffer)) {
    thing = thing.buffer;
...
function coerceToBase64Url(thing, name) {
  thing = coerceToBase64(thing, name); // base64 to base64url
...
function coerceToArrayBuffer(buf, name) {
  ...
  if (typeof buf === "string") {
    // base64url to base64
    buf = buf.replace(/-/g, "+").replace(/_/g, "/"); // base64 to Buffer
...
function checkOrigin(str) {
  var originUrl = URL.parse(str);
  var origin = `${originUrl.protocol}//${originUrl.hostname}${originUrl.port ? `:${originUrl.port}` : ""}`;
