...
class MdsEntry {
  ...
  constructor(mdsEntry, tocEntry) {
    ...
    if (this.protocolFamily === undefined) this.protocolFamily = "uaf"; // fix boolean values, since NNL doesn't validate them very well

    realBoolean(this, "isSecondFactorOnly");
    realBoolean(this, "isKeyRestricted");
    realBoolean(this, "isFreshUserVerificationRequired"); // TODO: read spec for other values
  }
...
class MdsCollection {
  ...
  addToc(tocStr, rootCert, crls) {
    ...
    return _asyncToGenerator(function* () {
      ...
      try {
        parsedJws = yield jose.JWS.createVerify().verify(tocStr, {
          allowEmbeddedKey: true
        });
        _this.toc = JSON.parse(ab2str(coerceToArrayBuffer(parsedJws.payload, "MDS TOC payload")));
      } catch (e) {
      ...
      var rootCerts;
      if (Array.isArray(rootCert)) rootCerts = rootCert;else rootCerts = [rootCert];
      var ret = yield CertManager.verifyCertChain(parsedJws.header.x5c, rootCerts, crls); // save the raw TOC

      _this.toc.raw = tocStr;
      return _this.toc;
    })();
  }
  /**
  ...
  validate() {
    ...
    this.unvalidatedEntryList.forEach(entry => {
      ...
      tocEntry = tocEntry[0]; // validate hash

      const hash = crypto.createHash("sha256"); // coerceToArrayBuffer(entry.raw, "MDS entry")
