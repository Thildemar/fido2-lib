...
class Fido2Lib {
  ...
  */
  constructor(opts) {
    ...
    checkOptType(this.config, "rpId", "string"); // rpName

    this.config.rpName = opts.rpName || "Anonymous Service";
    checkOptType(this.config, "rpName", "string"); // rpIcon

    this.config.rpIcon = opts.rpIcon;
    checkOptType(this.config, "rpIcon", "string"); // authenticatorRequireResidentKey

    this.config.authenticatorRequireResidentKey = opts.authenticatorRequireResidentKey;
    checkOptType(this.config, "authenticatorRequireResidentKey", "boolean"); // authenticatorAttachment
  ...
  attestationOptions(opts) {
    ...
    return _asyncToGenerator(function* () {
      opts = opts || {}; // The object being returned is described here:
      // https://w3c.github.io/webauthn/#dictdef-publickeycredentialcreationoptions

      var challenge = crypto.randomBytes(_this.config.challengeSize);
      challenge = coerceToArrayBuffer(challenge, "challenge");
      var pubKeyCredParams = [];

      _this.config.cryptoParams.forEach(coseId => {
      ...
      var extensions = createExtensions.call(_this, "attestation", opts.extensionOptions);
      /**
      ...
      setOpt(options.rp, "name", _this.config.rpName);
      setOpt(options.rp, "id", _this.config.rpId);
      setOpt(options.rp, "icon", _this.config.rpIcon);
      setOpt(options, "challenge", challenge);
      setOpt(options, "pubKeyCredParams", pubKeyCredParams);
      setOpt(options, "timeout", _this.config.timeout);
      setOpt(options, "attestation", _this.config.attestation);

      if (_this.config.authenticatorAttachment !== undefined || _this.config.authenticatorRequireResidentKey !== undefined || _this.config.authenticatorUserVerification !== undefined) {
        options.authenticatorSelectionCriteria = {};
        setOpt(options.authenticatorSelectionCriteria, "attachment", _this.config.authenticatorAttachment);
        setOpt(options.authenticatorSelectionCriteria, "requireResidentKey", _this.config.authenticatorRequireResidentKey);
        setOpt(options.authenticatorSelectionCriteria, "userVerification", _this.config.authenticatorUserVerification);
      }
  ...
  assertionOptions(opts) {
    ...
    return _asyncToGenerator(function* () {
      opts = opts || {}; // https://w3c.github.io/webauthn/#dictdef-publickeycredentialcreationoptions

      var challenge = crypto.randomBytes(_this2.config.challengeSize);
      challenge = coerceToArrayBuffer(challenge, "challenge");
      var options = {}; // mix extraData into challenge
      ...
      var extensions = createExtensions.call(_this2, "assertion", opts.extensionOptions);
      /**
      ...
      setOpt(options, "challenge", challenge);
      setOpt(options, "timeout", _this2.config.timeout);
      setOpt(options, "rpId", _this2.config.rpId);
      setOpt(options, "userVerification", _this2.config.authenticatorUserVerification);
      setOpt(options, "rawChallenge", rawChallenge);
  ...
  static parseAttestation(fmt, attStmt) {
    ...
    var ret = fmtObj.parseFn.call(this, attStmt); // validate return
  ...
  static validateAttestation() {
    ...
    return _asyncToGenerator(function* () {
      var fmt = _this3.authnrData.get("fmt"); // validate input
      ...
      var ret = yield fmtObj.validateFn.call(_this3); // validate return
...
function checkOptType(opts, prop, type) {
  ...
  if (opts[prop] === undefined) return; // native type
...
function createExtensions(type, extObj) {
  ...
  var extensions = {}; // default extensions

  var enabledExtensions = this.extSet;
  var extensionsOptions = this.extOptMap; // passed in extensions
...
Fido2Lib.addAttestationFormat(noneAttestation.name, noneAttestation.parseFn, noneAttestation.validateFn); // add 'fido-u2f' attestation format

const u2fAttestation = require("./attestations/fidoU2F");

Fido2Lib.addAttestationFormat(u2fAttestation.name, u2fAttestation.parseFn, u2fAttestation.validateFn); // add 'packed' attestation format

const packedAttestation = require("./attestations/packed");

Fido2Lib.addAttestationFormat(packedAttestation.name, packedAttestation.parseFn, packedAttestation.validateFn); // add 'tpm' attestation format

const tpmAttestation = require("./attestations/tpm");

Fido2Lib.addAttestationFormat(tpmAttestation.name, tpmAttestation.parseFn, tpmAttestation.validateFn); // add 'android-safetynet' attestation format
